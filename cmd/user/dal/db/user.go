...
type User struct {
	...
	Username  string         `gorm:"size:255;not null"`                        // 定义varchar字段长度为100
	Password  string         `gorm:"size:255;not null"`                        // 同上
	Avatar    string         `gorm:"size:255"`                                 // 可以为空
	OtpSecret string         `gorm:"size:255;column:otp_secret"`               // 注意这里是opt_secret，看起来可能是一个笔误，如果确实是otp_secret，请做相应调整
